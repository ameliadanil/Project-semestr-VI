Pamiętaj, aby rozwiązania do zadań umieszczać w 
...
- pobierz, następnie wyświetl wszystkie parametry w jednej linii. np.:
```
1 John 4 Kowalski 
```
lub
...
Pamiętaj, że ilość parametrów może być dowolna, niezależnie od tego program powinien działać prawidłowo.
...
Kowalski 4 John 1
```
dla zadanych `one two ` jako wynik otrzymamy
```
two one
```
